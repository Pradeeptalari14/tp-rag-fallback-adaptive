# 🚦 rag-fallback (Adaptive Thresholding)

> Dynamic similarity search and reranker thresholds tuned dynamically based on query profile scores.
...
## 🎛️ How It Works
Open the **[Interactive Studio](https://pradeeptalari14.github.io/portfolio/tools/rag-fallback-adaptive/)** in your portfolio dashboard to customize options, select webhook notification channels (Slack/Teams), toggle chat bot connections (WhatsApp Business/Telegram Bot), and download compiled deployment scripts dynamically.
...
```bash
git clone https://github.com/Pradeeptalari14/tp-rag-fallback-adaptive.git
cd tp-rag-fallback-adaptive
```
...
```bash
kubectl port-forward deployment/tp-rag-fallback-adaptive 8080:8080 || echo "Port forwarding enabled on localhost:8080"
```
...
└── primary_config/
    └── adaptive_thresholds.py     # Main configuration file
```
...
---
*Generated by [rag-fallback (Adaptive Thresholding) Studio](https://pradeeptalari14.github.io/portfolio/tools/rag-fallback-adaptive/) — [Talari Pradeep SRE Portfolio](https://pradeeptalari14.github.io/portfolio)*
